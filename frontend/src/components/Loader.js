...
const circle = styled.div`
  ...
  border-radius: 100%;
  position: absolute;
  animation: ${loadDelay} ${(props) => props.speed || 1.2}s infinite ease-in-out;
...
const CircleOne1 = styled(circle)`
  top: 0;
  left: 0;
...
const CircleOne2 = styled(circle)`
  top: 0;
  left: 0;
  animation-delay: -1s;
`;
...
const Loader = ({ style = commonStyle, color, size = "default", speed }) => {
  return (
    <LoadContainer style={style}>
      <ContainerOne>
        <CircleOne color={color} size={size} speed={speed} />
      ...
      <ContainerTwo>
        <CircleOne1 color={color} size={size} speed={speed} />
      ...
      <ContainerThree>
        <CircleOne2 color={color} size={size} speed={speed} />
